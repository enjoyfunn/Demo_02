...
function saveUser($con, $username, $password)
{
    ...
    $ajaxReturn = [];
    if ($result) {
        $ajaxReturn = ["code" => "200","msg" => "数据保存成功"];
    } else {
        $ajaxReturn = ["code" => "0","msg" => "数据保存失败"];
    }
...
function checkLogin()
{
    ...
    if (isset($_SESSION["user"])) {
        return json_encode(['code' => 200, 'msg' => '⽤户已登录', 'status'=> true], JSON_UNESCAPED_UNICODE);
    } else {
        return json_encode(['code' => 0, 'msg' => '⽤户未登录', 'status' =>false], JSON_UNESCAPED_UNICODE);
    }
}
